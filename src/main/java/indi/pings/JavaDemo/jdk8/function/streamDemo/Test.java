package indi.pings.JavaDemo.jdk8.function.streamDemo;

import java.util.Arrays;
import java.util.List;

import static java.util.stream.Collectors.toList;
import static java.util.stream.Collectors.joining;
import static java.util.Comparator.comparing;

/**
 *********************************************************
 ** @desc  ：  测试                                           
 ** @author  Pings                                     
 ** @date    2017年11月27日  
 ** @version v1.0                                                                                  
 * *******************************************************
 */
public class Test {

	public static void main(String[] args) {
		Trader raoul = new Trader("Raoul", "Cambridge");
		Trader mario = new Trader("Mario", "Milan");
		Trader alan = new Trader("Alan", "Cambridge");
		Trader brian = new Trader("Brian", "Cambridge");
		List<Trader> traders = Arrays.asList(raoul, mario, alan, brian);
		
		List<Transaction> transactions = Arrays.asList(new Transaction(brian, 2011, 300),
				new Transaction(raoul, 2012, 1000), new Transaction(raoul, 2011, 400),
				new Transaction(mario, 2012, 710), new Transaction(mario, 2012, 700), 
				new Transaction(alan, 2012, 950));
		
		//**(1) 找出2011年发生的所有交易，并按交易额排序（从低到高）
		List<Transaction> list1 = transactions.stream().filter(transaction -> transaction.getYear() == 2011)
				.sorted(comparing(Transaction::getValue)).collect(toList());
		//**(2) 交易员都在哪些不同的城市工作过
		List<String> list2 = traders.stream().map(Trader::getCity).distinct().collect(toList());
		//**(3) 查找所有来自于剑桥的交易员，并按姓名排序
		List<Trader> list3 = traders.stream().filter(trader -> trader.getCity().equals("Cambridge")).distinct()
				.sorted(comparing(Trader::getName)).collect(toList());
		//**(4) 返回所有交易员的姓名字符串，按字母顺序排序
		String str = traders.stream().map(Trader::getName).distinct().sorted().collect(joining());
		//**(5) 有没有交易员是在米兰工作的
		boolean is = traders.stream().anyMatch(trader -> trader.getCity().equals("Milan"));
		//**(6) 打印生活在剑桥的交易员的所有交易额
		transactions.stream().filter(transaction -> transaction.getTrader().getCity().equals("Cambridge"))
				.map(Transaction::getValue).forEach(System.out::println);
		//**(7) 所有交易中，最高的交易额是多少
		int max = transactions.stream().map(Transaction::getValue).reduce(Integer::max).get();
		int max1 = transactions.stream().max(comparing(Transaction::getValue)).get().getValue();
		//**(8) 找到交易额最小的交易
		int min =transactions.stream().map(Transaction::getValue).reduce(Integer::min).get();
		
		System.out.println(list1);
		System.out.println(list2);
		System.out.println(list3);
		System.out.println(str);
		System.out.println(is);
		System.out.println(max + ":" + max1);
		System.out.println(min);
	}
}
